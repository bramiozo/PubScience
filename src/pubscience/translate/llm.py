...
class llm_input(BaseModel):
    ...
    def __str__(self) -> str:
        return (
            "{"
            + f"'source_language': '{self.source_language}', 'target_language': '{self.target_language}', 'text_to_translate': '{self.text_to_translate}'"
            + "}"
        )
...
class llm_inputs(BaseModel):
    ...
    def __repr__(self) -> str:
        return (
            f"llm_inputs(source_language={self.source_language!r}, "
...
class TranslationLLM:
    def __init__(
        ...
        if system_prompt != "":
            ...
            try:
                ...
                self.system_prompt = llm_settings["translation"]["method"]["llm"][
                    "system_prompt"
                ]
        ...
        if provider == "openai":
            ...
            if model not in [m.id for m in self.client.models.list()]:
                raise ValueError(
                    f"Model {model} not available. Allowable models are: {self.client.models.list()}"
                )
            ...
            if model not in anthropic_models:
                raise ValueError(
                    f"Model {model} not available. Allowable models are: {anthropic_models}"
                )
            ...
            if model not in groq_models:
                raise ValueError(
                    f"Model {model} not available. Allowable models are: {groq_models}"
                )
            ...
            if model != "deepseek-chat":
                raise ValueError(
                    f"Model {model} not available. Allowable models are: ['deepseek-chat']"
                )
        elif provider == "local":
            # EuroLLM-9B-Instruct
            ...
            self.client, self.tokenizer = FastLanguageModel.from_pretrained(
                model_name=model, max_seq_length=max_tokens, load_in_4bit=True
            )
    ...
    def _get_anthropic_models(self) -> List[str]:
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Failed to get models from Anthropic API: {response.status_code} {response.text}"
            )
    ...
    def _get_groq_models(self) -> List[str]:
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Failed to get models from Groq API: {response.status_code} {response.text}"
            )
    ...
    def translate(self, text: str) -> Dict[str, Any]:
        ...
        if len(text.split()) > self.max_tokens:
            raise ValueError(
                f"Text is too long. Max tokens is {self.max_tokens}. Text has {len(text.split())} tokens."
            )
    ...
    def translate_batch(self, texts: List[str]) -> List[Dict[str, Any]]:
        assert self.provider.lower() != "local", (
            "Batch translation not supported for the local LLM."
        )
        assert len(texts) <= self.max_processes, (
            f"Batch size {len(texts)} exceeds maximum number of processes: {self.max_processes}"
        )
        ...
        if self.max_tokens_truncate:
            ...
            assert all([len(text.split()) <= self.max_tokens for text in texts]), (
                f"One or more texts are too long. Max tokens is {self.max_tokens}."
            )

        warnings.warn(
            """\n\nUsing `translate_batch()` is 50% more expensive than using `llm_batch()`. \n The latter is also faster for larger amounts. \n\n Please consider using `llm_batch()` instead.""",
            stacklevel=2,
        )
        ...
        if self.provider in ["openai", "deepseek"]:
            coroutines = [
                self._translate_openai_async(input_text) for input_text in InputTexts
            ]
        elif self.provider == "anthropic":
            coroutines = [
                self._translate_anthropic_async(input_text) for input_text in InputTexts
            ]
        elif self.provider == "google":
            coroutines = [
                self._translate_google_async(input_text) for input_text in InputTexts
            ]
        elif self.provider == "groq":
            coroutines = [
                self._translate_groq_async(input_text) for input_text in InputTexts
            ]
        else:
            raise ValueError(f"Unsupported provider: {self.provider}")
    ...
    def _translate_local(self, InputText: llm_input) -> Dict[str, Any]:
        ...
        inputs = self.tokenizer(
            [prompt_format.format(self.system_prompt, _InputText, "")],
            return_tensors="pt",
        ).to(self.device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Translate annotated and non-annotated corpora using LLMs."
    )
    parser.add_argument(
        "--model", type=str, required=True, help="The model to use for translation."
    )
    parser.add_argument(
        "--provider",
        type=str,
        required=True,
        choices=["openai", "anthropic", "google", "groq", "local", "deepseek"],
        help="The engine to use for translation.",
    )
    parser.add_argument(
        "--source_lang",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional system prompt for the translation model.",
    )
    parser.add_argument(
        "--env_loc", type=str, default=".env", help="The location of the .env file."
    )
