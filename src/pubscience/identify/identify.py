...
class Identify:
    def __init__(
        ...
        inclusion_terms: List | None = None,
        exclusion_terms: List | None = None,
        model_threshold: float = 0.5,
        model_outcome: bool = True,
        device: str | int = "cuda",
        id_col: str | None = "id",
        text_col: str | None = "text",
        textfile_readlines: bool = False,
        file_splitter_regex: str | None = None,
        max_write_size: int | None = None,
        max_read_size: int = 512 * 1024,
        max_chunk_length: int = 256,
        write_interval: int = 128,
        batch_size: int = 64,
        output_file: str = "./output/output.jsonl",
        streaming: bool = False,
        min_length: int = 64,
        start_from_previous: bool = False,
        greedy: bool = False,
        ...
        self.file_splitter_regex = (
            re.compile(file_splitter_regex) if file_splitter_regex else None
        )
        self.max_write_size = (
            max_write_size if max_write_size is not None else max_read_size
        )
        ...
        if len(self.existing_ids) == 0:
            ...
            print(
                f"Found: {len(self.existing_ids)} prior processed ids, skipping these..",
                flush=True,
            )
    ...
    def load_model(model_path, device):
        ...
        return pipeline(
            "text-classification", model=model_path, device=device, truncation=True
        )
    ...
    def _term_inclusion(self, text):
        ...
        if self.conservative:
            return any(term in text for term in self.inclusion_terms) and (
                not any(term in text for term in self.exclusion_terms)
            )
        else:
            return not any(term in text for term in self.exclusion_terms)

    def _model_inclusion(self, text: str) -> float:
    ...
    def _model_inclusion_batch(self, texts: List[str]) -> List[float]:
        # Check if the model predicts inclusion
        if not self.model:
            return [True]
    ...
    def is_relevant(self, text) -> Tuple[bool, float]:
        ...
        model_score = self._model_inclusion(text)
        if self.greedy:
            return term_relevant or (model_score > self.model_threshold), model_score
        else:
            return term_relevant and (model_score > self.model_threshold), model_score

    def is_relevant_batch(self, texts: List[str]) -> List[Tuple[bool, float]]:
        # Determine if a text is relevant based on terms and/or model
        ...
        model_scores = self._model_inclusion_batch(texts)
        model_relevant = [score > self.model_threshold for score in model_scores]
        if self.greedy:
            return [
                ((t[0] == True) or (t[1] == True), t[2])
                for t in zip(term_relevant, model_relevant, model_scores)
            ]
        else:
            return [
                ((t[0] == True) and (t[1] == True), t[2])
                for t in zip(term_relevant, model_relevant, model_scores)
            ]
                    ...
                    for line in file:
                        ...
                        if self.file_splitter_regex is not None:
                            ...
                            if match:
                                split_match = (
                                    match.group()
                                )  # Get the actual text that matched
                        ...
                        if split_match or (bytes_read >= self.max_read_size):
                            ...
                            if split_match:
                                ...
                                clean_match = re.sub(r"[^a-zA-Z0-9]", "", split_match)[
                                    :20
                                ]
                            ...
                            if self.is_relevant(accumulated_text)[0]:
                                self.included_documents.append(
                                    {"id": doc_id, "text": accumulated_text}
                                )
            ...
            for elem in root.iter():
                if elem.text and elem.text.strip():
                    ...
                    if self.id_col is not None:
                        doc_id = (
                            elem.get(self.id_col)
                            if elem.get(self.id_col)
                            else str(hash(text))
                        )
                    else:
                        doc_id = str(hash(text))
    ...
    def parse_dataset(
        ...
        dataset = load_dataset(dataset, split=split, streaming=True, **kwargs)
        texts_batch = []
        texts_full = []
        doc_ids = []
        text_lens = []
        ...
        for item in tqdm(dataset):
            ...
            if text_len >= self.min_length:
                ...
                if len(self.included_documents) > self.write_interval:
                    print(
                        f"Writing {self.write_interval} documents to disk...average text_len: {sum(text_lens) / len(text_lens)}"
                    )
                    self.write_documents_to_disk()
                    self.included_documents = []
                ...
                if aggregation == "simple":
                    if len(text) > self.max_chunk_length:
                        max_len_text = " ".join(
                            text.split(" ")[: self.max_chunk_length]
                        )
                    else:
                        max_len_text = text
                    ...
                    texts_full += [text]
                    text_full_len += [text_len]

                    if len(texts_batch) >= self.batch_size:
                        scores = self.is_relevant_batch(texts_batch)
                        for k, scoreTuple in enumerate(scores):
                            if scoreTuple[0] == True:
                                self.included_documents.append(
                                    {
                                        "id": doc_ids[k],
                                        "text": texts_full[k],
                                        "score": scoreTuple[1],
                                        "text_len": text_full_len[k],
                                    }
                                )
                        texts_batch = []
                        texts_full = []
                    ...
                    raise NotImplementedError(
                        "Majority aggregation not yet implemented"
                    )
                    votes = []
                    ...
                    for i in range(len(word_list) // self.max_chunk_length):
                        chunk = " ".join(
                            word_list[
                                i * self.max_chunk_length : (i + 1)
                                * self.max_chunk_length
                            ]
                        )
                        if self.is_relevant(chunk)[0]:
                            votes.append(1)
                        else:
                            votes.append(0)
                    ...
                    raise NotImplementedError(
                        "Selective aggregation not yet implemented"
                    )
                else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Identify relevant content in documents"
    )
    ...
    parser.add_argument(
        ...
        default=32,
        type=int,
        help="Minimum number of words of document..",
    )
    parser.add_argument(
        "--model_threshold",
        default=0.5,
        type=float,
        help="Minimum model proba for positive..",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--conservative", action="store_true", help="Enable conservative mode"
    )
    parser.add_argument(
        "--subset", type=str, default="", help="Select subset from data"
    )
    ...
    if args.dataset:
        ...
        if args.subset != "":
            kw = {"name": args.subset}
        identifier.parse_dataset(args.dataset, kwargs=kw)
