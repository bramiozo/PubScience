...
class LLMOutput(BaseModel):
    ...
    logprob: Optional[float] = Field(
        default=None, description="Log probability of the response"
    )
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional metadata from the API response"
    )
...
class extract:
    def __init__(
        ...
        # TODO: Make api version settable for openai; chat.completions versus responses
        # check availability of responses api here: https://learn.microsoft.com/en-us/azure/ai-foundry/openai/how-to/responses?view=foundry-classic&tabs=python-secure#region-availability
        if (any([c in model.lower() for c in ["gpt-5", "o4", "o3", "o1"]])) and (
            provider == "openai"
        ):
        ...
        if (
            all(isinstance(i, str) for i in instruction_list) & len(instruction_list)
            > 0
        ):
            self.instruction_list = instruction_list
        else:
        ...
        if isinstance(provider, str) == False:
            try:
                ...
                raise ValueError(
                    f"Could not parse provider from yaml: {e}. Please identify an available provider."
                )
        ...
        if provider == "openai":
            if "azure" in os.getenv("OPENAI_LLM_API_BASE").lower():
                ...
                self.client = AzureOpenAI(
                    azure_endpoint=os.getenv("OPENAI_LLM_API_BASE"),
                    api_version=os.getenv("OPENAI_LLM_API_VERSION", "2025-06-01"),
            ...
            if model_base is None:
                raise ValueError(
                    f"Model {model} does not match our supported unsloth base models: {unsloth_base_models}. Please specify a valid model."
                )

            if CHAT_TEMPLATES.get(model_base, None) is None:
                raise ValueError(
                    f"Chat template for model {model} not found in CHAT_TEMPLATES. Available templates: {list(CHAT_TEMPLATES.keys())}"
                )
            ...
            self.client, self.tokenizer = FastLanguageModel.from_pretrained(
                model_name=model, max_seq_length=max_tokens, load_in_4bit=four_bit
            )
            self.tokenizer = get_chat_template(
                self.tokenizer, chat_template=model_base
        ...
        try:
            ...
            if hasattr(response, "candidates") and response.candidates:
                ...
                if hasattr(candidate, "log_probs") and candidate.log_probs:
                    logprob = sum(
                        [token.log_probability for token in candidate.log_probs.tokens]
                    )
    ...
    def __transform_openai(self, InputText: llm_input) -> LLMOutput:
        ...
        metadata = {
            "finish_reason": choice.finish_reason,
            "usage": {
                "prompt_tokens": response.usage.prompt_tokens,  # input_tokens
                "completion_tokens": response.usage.completion_tokens,  # output_tokens
            },
        }
    ...
    def __transform_local(self, InputText: llm_input) -> LLMOutput:
        ...
        metadata = {
            "sequence_length": len(response.sequences[0])
            if hasattr(response, "sequences")
            else None,
...
def parse_folder_with_txt(arguments: argparse.Namespace):
    ...
    already_parsed = [
        re_replace_steps.sub("", fn.replace("_transformed", ""))
                ...
                with open(out_path, "a", encoding="utf-8") as f:
                    for k, _trans in enumerate(_transformer.intermediate_outputs):
                        json_line = json.dumps(
                            {arguments.id_field: id, "k": k, "extraction": _trans}
                        )
                        f.write(json_line + "\n")
                    sleep(1)
            except Exception as e:
...
def parse_jsonl(arguments: argparse.Namespace):
    ...
    with open(arguments.input_path, "r", encoding="utf-8") as f:
        ...
        for d in tqdm(list_of_dicts):
            ...
            text = "\n".join(
                [d[tfield] for tfield in arguments.text_fields if tfield in d]
            )
        ...
        for idx, text in tqdm(enumerate(lines)):
            ...
            try:
                ...
                with open(out_path, "a", encoding="utf-8") as f:
                    for k, _trans in enumerate(_transformer.intermediate_outputs):
                        json_line = json.dumps(
                            {"id": idx, "k": k, "transformed_text": _trans}
                        )
                        f.write(json_line + "\n")
                    sleep(1)
            except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_tokens", type=int, help="Maximum tokens for the LLM", default=5_000
    )
    parser.add_argument(
